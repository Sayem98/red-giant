...
export const Home = () => {
    ...
    const getTokenMetadata = async ( tokens ) => {
        ...
        if(tokenData.address == undefined) return;


        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_TOKEN_ADDRESS ) )
            tokenData.logo = redg_logo;
        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_CREED_ADDRESS ) )
            tokenData.logo = creed_image;
        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_ACAKE47_ADDRESS ) )
            tokenData.logo = acake47_image;
        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_PEPED_ADDRESS ) )
            tokenData.logo = peped_image;
        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_REDR_ADDRESS ) )
            tokenData.logo = redr_image;
        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_PANGEA_ADDRESS ) )
            tokenData.logo = pangea_image;
        if ( toPlainString( tokenData.address ) === toPlainString( process.env.REACT_APP_MAN_ADDRESS ) )
    ...
    async function getEvents() {
        ...
        let pool_array = [];
        for ( var i = 0; i < fixed_logs.result.length; i++ ) {
            const log = fixed_logs.result[ i ];
            
            try {
                const data = web3_inline.eth.abi.decodeLog( inputs.deployedERC20FarmFixEnd, log.data, [ topics.erc20FixEnd ] );
                if ( data._startBlock < PLATFORM_START_DATE ) continue;
                ...
                if ( stakeTokenData.symbol.toLowerCase().includes( "-lp" ) ) {
                    const lpContract = new web3_inline.eth.Contract( lpAbi, stakeTokenData.address );
                    ...
                    const tokenContract = new web3_inline.eth.Contract( tokenAbi, rewardTokenData.address );
                    const tokenBalance = await tokenContract.methods.balanceOf( data.farmAddress ).call();
                    const tokenPrice = await getTokenPrice( rewardTokenData.address );
                    // const tokenPrice = { usdPrice: 0.269 };
                    const farmPrice = tokenPrice.usdPrice * ( tokenBalance / 10 ** rewardTokenData.decimals );
                    const mainTokenPrice = await getTokenReserve( stakeTokenData.address, token0Data.decimals, token1Data.decimals );
        ...
        console.log( logs.result.length );
        for ( var i = 0; i < logs.result.length; i++ ) {
            const log = logs.result[ i ];
            try {
                const data = web3_inline.eth.abi.decodeLog( inputs.deployedERC20Farm, log.data, [ topics.erc20 ] );
